...
test('admin order export api creates completed csv exports', function (): void {
    ...
    $readToken = adminApiBearerToken($store, ['read-orders'], adminOrderExportApiUser());

    $createResponse = $this->withToken($readToken)
    ...
    $showResponse = $this->withToken($readToken)
...
test('admin order export api enforces token abilities and store scope', function (): void {
    ...
    $this->withToken(adminApiBearerToken($store, ['read-orders'], adminOrderExportApiUser()))
