            ...
            <div class="rounded-lg border border-zinc-200 p-5 dark:border-zinc-800">
                <dl class="flex flex-col gap-2 text-sm">
                    <div class="flex justify-between gap-4">
                        <dt>Subtotal</dt>
                        <dd>@include('storefront.components.price', ['amount' => $cart->subtotalAmount(), 'currency' => $cart->currency])</dd>
                    </div>
                    @if($cart->discountAmount() > 0)
                        <div class="flex justify-between gap-4">
                            <dt>Discount</dt>
                            <dd>-@include('storefront.components.price', ['amount' => $cart->discountAmount(), 'currency' => $cart->currency])</dd>
                        </div>
                    @endif
                    <div class="flex justify-between gap-4 border-t border-zinc-200 pt-2 text-base font-semibold dark:border-zinc-800">
                        <dt>Total</dt>
                        <dd>@include('storefront.components.price', ['amount' => $cart->totalAmount(), 'currency' => $cart->currency])</dd>
                    </div>
                </dl>

                <form wire:submit="applyDiscount" class="mt-5 flex flex-col gap-3 sm:flex-row">
                    <input wire:model="discountCode" type="text" autocomplete="off" placeholder="Discount code" class="min-w-0 flex-1 rounded-md border border-zinc-300 bg-white px-3 py-2 text-sm text-zinc-950 dark:border-zinc-700 dark:bg-zinc-900 dark:text-white">
                    <button type="submit" class="rounded-md border border-zinc-300 px-5 py-2 text-sm font-semibold data-loading:opacity-60 dark:border-zinc-700">
                        Apply
                    </button>
                </form>

                @if($cart->discount_code)
                    <button type="button" wire:click="removeDiscount" class="mt-2 text-sm font-semibold text-zinc-700 underline dark:text-zinc-300">
