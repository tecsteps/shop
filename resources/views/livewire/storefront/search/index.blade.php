<div class="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
    ...
    <div class="mt-4 max-w-3xl">
        <h1 class="text-3xl font-bold tracking-tight text-gray-900 sm:text-4xl dark:text-white">
            @if (trim($query) !== '' && $products !== null)
                {{ $products->total() }} {{ str('result')->plural($products->total()) }} for &ldquo;{{ $query }}&rdquo;
            @elseif (trim($query) !== '')
        ...
        <form class="mt-6" role="search" @submit.prevent>
            ...
            <div class="relative">
                <svg class="pointer-events-none absolute left-3 top-1/2 size-5 -translate-y-1/2 text-gray-400" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true">
                ...
                <input id="search-page-input"
                       ...
                       autocomplete="off"
                       class="w-full rounded-md border border-gray-300 bg-white py-2.5 pl-10 pr-4 text-sm text-gray-900 placeholder-gray-400 focus:border-blue-500 focus:outline-hidden focus:ring-2 focus:ring-blue-500 dark:border-gray-700 dark:bg-gray-800 dark:text-white">
            </div>
        </form>
    </div>
    ...
    @if (trim($query) === '')
        <div class="flex flex-col items-center gap-3 py-24 text-center">
            ...
            <p class="text-lg font-semibold text-gray-900 dark:text-white">Search our store</p>
            <p class="max-w-md text-sm text-gray-500 dark:text-gray-400">Type a keyword above to find products.</p>
        </div>
    @else
        {{-- Toolbar --}}
        <div class="mt-8 flex items-center justify-between gap-4 border-y border-gray-200 py-3 dark:border-gray-800">
            ...
            <p class="hidden text-sm text-gray-500 lg:block dark:text-gray-400" aria-live="polite">
                {{ $products->total() }} {{ str('result')->plural($products->total()) }}
            </p>
        ...
        <div class="mt-8 flex gap-8" x-data="{ filtersOpen: false }" @toggle-filters.window="filtersOpen = !filtersOpen">
            ...
            <aside class="hidden w-64 shrink-0 lg:block" aria-label="Search filters">
                @include('livewire.storefront.search.partials.filters')
            </aside>
            ...
            <div class="min-w-0 flex-1" aria-live="polite">
                @if ($products->isEmpty())
                    <div class="flex flex-col items-center gap-3 py-24 text-center">
                        ...
                        <p class="text-lg font-semibold text-gray-900 dark:text-white">No results found for &ldquo;{{ $query }}&rdquo;</p>
                        <p class="max-w-md text-sm text-gray-500 dark:text-gray-400">Try a different search term or adjust your filters.</p>
                        @if (count($activeFilters) > 0)
