<section class="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
    ...
    <div class="mt-6 max-w-3xl space-y-3">
        <h1 class="text-3xl font-semibold tracking-normal text-zinc-950 dark:text-white">{{ $collection->title }}</h1>
        @if ($collection->description_html)
            <div class="text-zinc-600 dark:text-zinc-300">{!! $collection->description_html !!}</div>
        @endif
    </div>
                ...
                <div class="space-y-3">
                    ...
                    <div class="space-y-2">
                        @foreach ($productTypes as $type)
                            <flux:checkbox wire:model.live="types" value="{{ $type }}" :label="$type" wire:key="type-filter-{{ $type }}" />
                        @endforeach
                    </div>
                </div>
                ...
                <div class="space-y-3">
                    ...
                    <div class="space-y-2">
                        @foreach ($productVendors as $vendor)
                            <flux:checkbox wire:model.live="vendors" value="{{ $vendor }}" :label="$vendor" wire:key="vendor-filter-{{ $vendor }}" />
                        @endforeach
                    </div>
                </div>
            </div>
            ...
            @if ($products->isEmpty())
                <div class="rounded-lg border border-zinc-200 px-6 py-16 text-center dark:border-zinc-800">
                    <div class="mx-auto flex max-w-md flex-col items-center gap-3">
                        ...
                        <p class="text-zinc-600 dark:text-zinc-400">Try adjusting your filters or browse another collection.</p>
            ...
            @else
                <div class="grid grid-cols-2 gap-4 md:grid-cols-3" wire:loading.class="opacity-50">
                    @foreach ($products as $product)
                        <x-storefront.product-card :product="$product" wire:key="collection-product-{{ $product->getKey() }}" />
                    @endforeach
                </div>
