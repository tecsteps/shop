...
>
    <x-storefront.breadcrumbs :items="[
        ...
        ['label' => __('Collections'), 'url' => route('storefront.collections.index')],
    ...
    <header class="mt-4">
        ...
        @if (filled($collection->description_html))
            <div class="sf-prose mt-3 max-w-2xl">
    ...
    <div class="mt-8 flex items-center justify-between gap-4 border-y border-zinc-200 py-3 dark:border-zinc-800">
        ...
        <div class="flex items-center gap-2">
            <label for="collection-sort" class="text-sm text-zinc-500 dark:text-zinc-400">{{ __('Sort by') }}</label>
            <select
                id="collection-sort"
    ...
    @if ($hasActiveFilters)
        <div class="mt-4 flex flex-wrap items-center gap-2">
            @if ($inStock)
                <span class="inline-flex items-center gap-1.5 rounded-full bg-zinc-100 py-1 pr-1.5 pl-3 text-xs font-medium text-zinc-700 dark:bg-zinc-800 dark:text-zinc-300">
                    {{ __('In stock') }}
                    <button type="button" wire:click="$set('inStock', false)" class="rounded-full p-0.5 transition hover:bg-zinc-200 dark:hover:bg-zinc-700" aria-label="{{ __('Remove in stock filter') }}">
                        <svg class="size-3" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18 18 6M6 6l12 12" /></svg>
                    </button>
                </span>
            @endif
            @if ($priceMin !== '' || $priceMax !== '')
                <span class="inline-flex items-center gap-1.5 rounded-full bg-zinc-100 py-1 pr-1.5 pl-3 text-xs font-medium text-zinc-700 dark:bg-zinc-800 dark:text-zinc-300">
                    {{ __('Price') }}: {{ $priceMin !== '' ? $priceMin : '0' }} &ndash; {{ $priceMax !== '' ? $priceMax : '&infin;' }}
                    <button type="button" wire:click="$set('priceMin', ''); $set('priceMax', '')" class="rounded-full p-0.5 transition hover:bg-zinc-200 dark:hover:bg-zinc-700" aria-label="{{ __('Remove price filter') }}">
                        <svg class="size-3" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18 18 6M6 6l12 12" /></svg>
                    </button>
                </span>
            @endif
            @foreach ($productTypes as $activeType)
                <span class="inline-flex items-center gap-1.5 rounded-full bg-zinc-100 py-1 pr-1.5 pl-3 text-xs font-medium text-zinc-700 dark:bg-zinc-800 dark:text-zinc-300">
                    {{ $activeType }}
                    <button type="button" wire:click="$set('productTypes', {{ json_encode(array_values(array_diff($productTypes, [$activeType]))) }})" class="rounded-full p-0.5 transition hover:bg-zinc-200 dark:hover:bg-zinc-700" aria-label="{{ __('Remove :filter filter', ['filter' => $activeType]) }}">
                        <svg class="size-3" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18 18 6M6 6l12 12" /></svg>
                    </button>
                </span>
            @endforeach
            @foreach ($vendors as $activeVendor)
                <span class="inline-flex items-center gap-1.5 rounded-full bg-zinc-100 py-1 pr-1.5 pl-3 text-xs font-medium text-zinc-700 dark:bg-zinc-800 dark:text-zinc-300">
                    {{ $activeVendor }}
                    <button type="button" wire:click="$set('vendors', {{ json_encode(array_values(array_diff($vendors, [$activeVendor]))) }})" class="rounded-full p-0.5 transition hover:bg-zinc-200 dark:hover:bg-zinc-700" aria-label="{{ __('Remove :filter filter', ['filter' => $activeVendor]) }}">
                        <svg class="size-3" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18 18 6M6 6l12 12" /></svg>
                    </button>
                </span>
            @endforeach
            <button
                ...
                class="text-xs font-medium text-blue-700 transition hover:text-blue-800 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-600 dark:text-blue-400 dark:hover:text-blue-300"
            >
                {{ __('Clear all') }}
            </button>
        </div>
    ...
    <div class="mt-8 lg:grid lg:grid-cols-[16rem_1fr] lg:gap-10">
        ...
        {{-- Product grid --}}
        <div wire:loading.class="opacity-60" wire:target="sort, inStock, priceMin, priceMax, productTypes, vendors" class="transition-opacity">
            @if ($products->isEmpty())
                <div class="flex flex-col items-center justify-center py-24 text-center">
                    ...
                    <p class="mt-4 text-base font-semibold text-zinc-900 dark:text-white">{{ __('No products found') }}</p>
                    ...
                    @if ($hasActiveFilters)
                        <button
                            type="button"
                            wire:click="clearFilters"
                            class="mt-6 rounded-lg border border-zinc-300 px-4 py-2 text-sm font-semibold text-zinc-700 transition hover:bg-zinc-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-600 dark:border-zinc-700 dark:text-zinc-300 dark:hover:bg-zinc-800"
                        >
                            {{ __('Clear filters') }}
                        </button>
                    @endif
