            ...
            <tbody class="divide-y divide-gray-200 bg-white dark:divide-gray-700 dark:bg-gray-900">
                @forelse($collections as $collection)
                    <tr wire:key="collection-{{ $collection->id }}">
                        ...
                        <td class="px-4 py-3 text-sm text-gray-500">{{ $collection->products_count }}</td>
                        <td class="px-4 py-3">
                            <flux:badge size="sm" :color="$collection->status === 'published' ? 'green' : 'yellow'">{{ ucfirst($collection->status) }}</flux:badge>
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="3" class="px-4 py-8 text-center text-sm text-gray-500">No collections found.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
