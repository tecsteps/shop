<div>
    ...
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 mb-8">
        ...
        <div class="rounded-lg border border-zinc-200 bg-white p-6 dark:border-zinc-700 dark:bg-zinc-900">
            <flux:text class="text-zinc-500 dark:text-zinc-400">Top Products Tracked</flux:text>
            <flux:heading size="xl" class="mt-1">{{ count($topProducts) }}</flux:heading>
        </div>
    </div>
                    ...
                    <tbody>
                        @foreach ($topProducts as $product)
                            <tr class="border-b border-zinc-100 dark:border-zinc-800">
                                <td class="py-2 text-zinc-900 dark:text-zinc-100">{{ $product['dimension'] ?? 'Unknown' }}</td>
                                <td class="py-2 text-right text-zinc-600 dark:text-zinc-400">{{ number_format($product['total']) }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
