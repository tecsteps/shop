...
@php
    ...
    $subtotal = $totals['subtotal'] ?? $cart->lines->sum('line_subtotal_amount');
    $discount = $totals['discount'] ?? 0;
    $shipping = $totals['shipping'] ?? null;
    $tax = $totals['tax_total'] ?? null;
    $total = $totals['total'] ?? ($subtotal - $discount);
@endphp

<div {{ $attributes->class(['rounded-xl bg-zinc-50 p-6 dark:bg-zinc-900']) }}>
    <h2 class="text-base font-semibold text-zinc-900 dark:text-white">Order Summary</h2>

    <ul class="mt-4 space-y-3">
        @foreach ($cart->lines as $line)
            <li class="flex items-center gap-3" wire:key="summary-line-{{ $line->id }}">
        ...
        <div class="mt-4 border-t border-zinc-200 pt-4 dark:border-zinc-800">
            ...
            @else
                <form wire:submit.prevent="applyDiscount" class="flex gap-2">
                    ...
                    <flux:button type="submit" size="sm" variant="filled" wire:loading.attr="disabled" wire:target="applyDiscount">
    ...
    <dl class="mt-4 space-y-2 border-t border-zinc-200 pt-4 text-sm dark:border-zinc-800">
        <div class="flex justify-between">
            <dt class="text-zinc-500 dark:text-zinc-400">Subtotal</dt>
            <dd class="text-zinc-900 dark:text-white"><x-storefront.price :amount="$subtotal" :currency="$currency" /></dd>
        </div>
        ...
        <div class="flex justify-between border-t border-zinc-200 pt-2 text-base font-semibold text-zinc-900 dark:border-zinc-800 dark:text-white" aria-live="polite">
            <dt>Total</dt>
