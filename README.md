...
- There must be dedicated code review teammate which ensures the code follows clean code, SOLID and Laravel best practices.
- There must be dedicated QA Analyst that writes a full specs/testplan-{phase}.md for the current phase and then verifies functionality using Playwright and Chrome. The results of the regression test are tracked in the testplan. If bugs appear, then other teammates must fix them, so the QA Analyst can verify the fixes.
...
When all phases are developed, the teamlead spawns a dedicated fresh teammate to make a final verification using Playwright/Chrome based on the testplans of all phases. If bugs or gaps are detected, other teammates fix them. The controller must confirm everything is working.
