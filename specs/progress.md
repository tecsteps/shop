...
| Auth/security | `specs/06-AUTH-AND-SECURITY.md` | partial | Admin Livewire login/logout, customer guard/provider, customer login/register, customer password reset with store-scoped hashed tokens, reset-link throttling, generic reset responses, login rate limiting, session hardening, concrete resource policies, encrypted app client/webhook secrets, and one-time display developer token generation are implemented. Sanctum token auth is still missing. |
...
- Composer does not currently include Sanctum or Passport, while Spec 06 requires Sanctum personal access tokens and Spec 02 defers Passport OAuth. This will need either an approved dependency addition or a documented compatible alternative before final completion.
