...
- Status: in progress
- Active slice: Phase 7 - analytics/apps/webhooks
...
| Seed/test data | `specs/07-SEEDERS-AND-TEST-DATA.md` | partial | Seeders create Acme Fashion and Acme Electronics stores/domains/settings/admin access, 2 search settings rows, 25 indexed products, 6 collections, 127 variants, 127 inventory rows, 206 variant option pivots, 2 themes, 6 theme files, 2 theme settings rows, 6 pages, 4 navigation menus, 17 navigation items, 2 tax settings rows, 2 shipping zones, 6 shipping rates, 6 discounts, and no product media/runtime carts. Order/payment seed data is still missing. |
...
- Analytics event capture, app/webhook APIs, and broader admin REST endpoints outside order management are still missing.
