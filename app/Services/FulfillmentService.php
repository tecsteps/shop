    ...
    public function create(Order $order, array $lines, ?array $tracking = null): Fulfillment
    {
        ...
        return DB::transaction(function () use ($order, $lines, $tracking) {
            ...
            foreach ($lines as $orderLineId => $quantity) {
                ...
                $alreadyFulfilled = FulfillmentLine::where('order_line_id', $orderLineId)->sum('quantity');
                $unfulfilled = $orderLine->quantity - $alreadyFulfilled;
    ...
    private function guardFinancialStatus(Order $order): void
    {
        ...
        if (! in_array($order->financial_status, $allowed)) {
            throw new FulfillmentGuardException(
                "Cannot create fulfillment: order financial status is {$order->financial_status->value}."
            );
        }
    }
