...
class RefundService
{
    public function __construct(private readonly PaymentProvider $provider, private readonly InventoryService $inventory) {}
    ...
    public function create(Order $order, Payment $payment, int|array|null $amount = null, ?string $reason = null, bool $restock = false, array $lines = []): Refund
    {
        ...
        return DB::transaction(function () use ($order, $payment, $amount, $reason, $restock, $restockLines): Refund {
            ...
            $refund = $order->refunds()->create(['payment_id' => $payment->getKey(), 'amount' => $amount, 'status' => $result->successful ? 'processed' : 'failed', 'provider_refund_id' => $result->reference, 'reason' => $reason, 'restock' => $restock]);
