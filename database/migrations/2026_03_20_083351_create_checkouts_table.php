...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        DB::statement("CREATE TRIGGER checkouts_status_check BEFORE INSERT ON checkouts
            BEGIN
        ...
        DB::statement("CREATE TRIGGER checkouts_status_check_update BEFORE UPDATE ON checkouts
            BEGIN
                SELECT CASE WHEN NEW.status NOT IN ('started', 'addressed', 'shipping_selected', 'payment_selected', 'completed', 'expired')
                    THEN RAISE(ABORT, 'Invalid checkout status')
                END;
