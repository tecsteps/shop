...
return new class extends Migration
{
    public function up(): void
    {
        ...
        DB::statement("CREATE TRIGGER check_product_variants_status INSERT ON product_variants
            BEGIN
                SELECT CASE WHEN NEW.status NOT IN ('active', 'archived')
                    THEN RAISE(ABORT, 'Invalid variant status')
                END;
            END");

        DB::statement("CREATE TRIGGER check_product_variants_status_update UPDATE OF status ON product_variants
            BEGIN
                SELECT CASE WHEN NEW.status NOT IN ('active', 'archived')
                    THEN RAISE(ABORT, 'Invalid variant status')
                END;
    ...
    {
        DB::statement('DROP TRIGGER IF EXISTS check_product_variants_status');
        DB::statement('DROP TRIGGER IF EXISTS check_product_variants_status_update');
