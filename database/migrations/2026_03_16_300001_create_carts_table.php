...
return new class extends Migration
{
    public function up(): void
    {
        ...
        DB::statement("CREATE TRIGGER check_carts_status INSERT ON carts
            BEGIN
        ...
        DB::statement("CREATE TRIGGER check_carts_status_update UPDATE OF status ON carts
            BEGIN
                SELECT CASE WHEN NEW.status NOT IN ('active', 'converted', 'abandoned')
                    THEN RAISE(ABORT, 'Invalid cart status')
                END;
    ...
    {
        DB::statement('DROP TRIGGER IF EXISTS check_carts_status');
        DB::statement('DROP TRIGGER IF EXISTS check_carts_status_update');
