...
return new class extends Migration
{
    public function up(): void
    {
        ...
        DB::statement("CREATE TRIGGER check_products_status INSERT ON products
            BEGIN
        ...
        DB::statement("CREATE TRIGGER check_products_status_update UPDATE OF status ON products
            BEGIN
                SELECT CASE WHEN NEW.status NOT IN ('draft', 'active', 'archived')
                    THEN RAISE(ABORT, 'Invalid product status')
                END;
    ...
    {
        DB::statement('DROP TRIGGER IF EXISTS check_products_status');
        DB::statement('DROP TRIGGER IF EXISTS check_products_status_update');
