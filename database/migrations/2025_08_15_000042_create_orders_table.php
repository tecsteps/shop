...
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('orders', function (Blueprint $table) {
            ...
            $table->text('payment_method')->check("payment_method IN ('credit_card','paypal','bank_transfer')");
            $table->text('status')->default('pending')->check("status IN ('pending','paid','fulfilled','cancelled','refunded')");
            $table->text('financial_status')->default('pending')->check("financial_status IN ('pending','authorized','paid','partially_refunded','refunded','voided')");
            $table->text('fulfillment_status')->default('unfulfilled')->check("fulfillment_status IN ('unfulfilled','partial','fulfilled')");
            ...
            $table->unique(['store_id', 'order_number'], 'orders_store_order_number_unique');
